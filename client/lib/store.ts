...
export const store = {
  ...
  addItem: (listId: string, name: string, quantity: number): ShoppingItem | null => {
  ...
  updateItemQuantity: (listId: string, itemId: string, totalQuantity: number, acquiredQuantity: number): boolean => {
    const list = lists.get(listId)
  ...
  acquireItems: (listId: string, itemId: string, quantity: number): boolean => {
    ...
    item.acquiredQuantity = Math.min(item.acquiredQuantity + quantity, item.totalQuantity)
    return true
  },
